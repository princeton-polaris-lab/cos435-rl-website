...
<<<<<<< HEAD
\subsection*{Question 3: Thinking through a SARSA, Expected SARSA, and Q-Learning Update (10 points)}
=======
\subsection*{Question 3: Thinking through a SARSA, Expected SARSA, and Q-Learning Update (15 points)}
>>>>>>> e67bb2a (add assignment)
...
\]
<<<<<<< HEAD
with rewards $-1,\; -1,\; -100$ respectively. 
=======
...
<<<<<<< HEAD
\item \textbf{(6 pts)} Perform the \textbf{SARSA} updates for each of the three transitions. Show the updated $Q$-value after each step.
=======
...
<<<<<<< HEAD
\item \textbf{(6 pts)} Starting from the \emph{same} initial $Q$-values (not the values from part (a)), perform the \textbf{Q-Learning} updates for the same trajectory. Show each update and identify where the updates differ from SARSA.
=======
...
<<<<<<< HEAD
\item \textbf{(6 pts)} Again starting from the \emph{same} initial $Q$-values, perform the \textbf{Expected SARSA} updates. Show each update and identify where the updates differ from SARSA and Q-Learning.
=======
...
<<<<<<< HEAD
\item \textbf{(2 pts)} Compare the updated values of $Q(S,U)$ and $Q((1,0), R)$ across the three algorithms. In 2--3 sentences, explain intuitively why they differ, and relate this to the on-policy vs.\ off-policy distinction.
=======
